...
// Hardcoded API key for testing (remove after verification)
const API_KEY = "AIzaSyCC6IAi-s-nIIJVz-cIxAzOgRMjopjFUoc";
const ai = new GoogleGenAI({ apiKey: API_KEY || process.env.API_KEY });

export const analyzeProjects = async (projects: Project[]): Promise<string> => {
  // If key is removed later, keep guard
